...
class AdminLoginRequest(BaseModel):
    """
    🔥 Login admin unifié - accepte identifier (email OU téléphone)
    """
    identifier: str = None  # 🔥 Nouveau champ unifié
    phone_number: str = None  # Rétrocompatibilité
    password: str
    admin_code: str  # Code admin obligatoire
    
    def get_identifier(self) -> str:
        """Retourne l'identifiant (email ou téléphone)"""
        return self.identifier or self.phone_number or ""
...
async def admin_login(
    ...
    # 🔥 Récupérer l'identifiant (identifier > phone_number)
    identifier = data.get_identifier()
    if not identifier:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Identifiant requis (email ou téléphone)"
        )
    ...
    if data.admin_code != ADMIN_MASTER_PASSWORD:
        ...
        admin_logger.log_security_event(
            ...
            ip=ip,
            description=f"Tentative avec code admin invalide: {identifier[:4]}***"
        )
        raise HTTPException(
    ...
    # 2. 🔍 Vérifier l'utilisateur par identifier (email OU téléphone)
    user = get_user_by_identifier(db, identifier)
    if not user:
        rate_limiter.record_failed_login(request, "/admin/auth/login")
        admin_logger.log_auth_failed(identifier, ip, "user_not_found")
        raise HTTPException(
